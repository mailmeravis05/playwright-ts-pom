...
import {ReadCsv} from '../lib/csvUtils';

const testData = ReadCsv('./data/dataDriven.csv');
...
test.describe('Origin Energy - plans flow - Data Driven', () => {
  ...
  testData.forEach((data: any) => {
    ...
    if (!run || run?.trim().toLowerCase() !== "true") return; // Skip test if run is false or not "true"
